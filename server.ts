  ...
  app.post('/api/generate-questions', async (req, res) => {
    try {
      ...
      const ai = new GoogleGenAI({ apiKey });
                  ...
                  const promptCriticalThinking = `# Context
...
1. **Simplicidade:** Use vocabulário do cotidiano e construa frases muito curtas. A linguagem deve ser amigável e acessível.  
2. **Foco Único:** O enunciado deve ser extremamente claro e pedir **apenas uma coisa** ao aluno ao final da leitura (uma única reflexão, escolha ou passo).  
3. **Jornada do Estudante:** O foco das perguntas deve ser 100% no processo e na jornada do estudante. **Não inclua nenhum tipo de competição entre alunos, rankings ou comparações.**  
4. **Sem Certo ou Errado:** A formulação deve transmitir a ideia de que não há resposta certa ou errada absoluta. O foco é a reflexão e o raciocínio individual.  
5. **Essência da Avaliação:** Modifique o cenário usando os dados do aluno, mas mantenha intacta a habilidade cognitiva que a pergunta original exige (Análise, Tomada de Decisão, Metacognição, Argumentação, Transferência).
## PERGUNTAS-BASE (O que deve ser avaliado):
1. *Avaliação de Evidências (Análise e Avaliação):* Identificar premissas, conclusões e critérios lógicos de argumentos opostos suspendendo o julgamento emocional imediato.  
2. *Dilemas e Emoções (Tomada de Decisão):* Apresentar um dilema emocional/ético pessoal e descrever como gerenciou a tensão entre emoção e lógica.  
3. *Vieses Cognitivos (Autorregulação):* Explicar como mitigar o viés de confirmação e manter a honestidade intelectual perante opiniões opostas.  
4. *Articulação de Argumento (Explicação):* Defender um ponto de vista com conclusão clara, premissas evidentes e justificativa lógica das conexões.  
5. *Transferência de Aprendizado (Resolução de Problemas):* Usar um método estruturado (ex: método científico/teste de hipóteses) para resolver um problema não rotineiro da comunidade.
# FORMATO DE SAÍDA (OUTPUT):
Gere apenas as 5 perguntas numeradas, sem introduções teóricas. Escreva conversando diretamente com o aluno, integrando os dados dele de forma natural.
*Exemplo de estilo de saída (focado em análise e avaliação sem certo/errado):* ‘Imagine que você e seus amigos da escola \[Nome da Escola\] montaram uma banda de \[Música\], mas, bem na hora do primeiro show, a energia acaba em \[Cidade\] inteira. Sem usar a internet ou celulares, me conte a sua ideia mais criativa para resolver esse problema e fazer o show acontecer no escuro.’`;
